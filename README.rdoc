...
Use Arpeggiator#add and Arpeggiator#remove to change the notes that the arpeggiator sees.  Here are some possibilities 

  arp.add(chord)
  arp.add("C5")
  arp.remove(["C5", "A4"])
  arp.add(MIDIMessage::NoteOn["A4"].new(0, 64))
  # etc
...
* {Example of feeding notes to Diamond using a MIDI controller}
* {Example of feeding notes to Diamond using midi-message objects} 
