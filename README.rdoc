...
* Features all classic arpeggiator functions
...
* Sync between multiple instances as well as other {Topaz Tempo}[http://github.com/arirusso/topaz] objects
...
The Arpeggiator has a number of options to control which you can read about more in-depth {here}[http://tx81z.blogspot.com/].  Here is a pretty basic setup: 
...
If you are using this example in irb, you can start the arpeggiator in a background thread by passing an option :background => true to Arpeggiator#start.  That will allow you to {live code}[http://en.wikipedia.org/wiki/Live_coding], sync it to another arpeggiator, or just run another foreground process. 
...
{This blog post}[http://tx81z.blogspot.com] and the following screencast go more in-depth on Diamond usage, especially with live coding and the various kinds of sync available.
