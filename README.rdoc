...
First, select a MIDI output using {unimidi}[http://github.com/arirusso/unimidi].  ({here is an example that explains how to find a device}[http://github.com/arirusso/unimidi/blob/master/examples/select_a_device.rb])

  @output = UniMIDI::Output.first.open

The Arpeggiator has a number of optional parameters and you can read about what they do {here}[http://tx81z.blogspot.com/].  For this example, I'll use a very straightforward setup:
...
If you are working in irb, you can start the arpeggiator in a background thread by passing an option :background =&gt; true to Arpeggiator#start.  That will allow you to {live code}[http://en.wikipedia.org/wiki/Live_coding], sync it other arpeggiators, or just run another foreground process. 
