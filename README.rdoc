...
First, select a MIDI output using {unimidi}[http://github.com/arirusso/unimidi].  ({More...}[http://github.com/arirusso/unimidi/blob/master/examples/select_a_device.rb])
...
Of course, an Arpeggiator always needs notes to work with.  As you might expect, it is easy to use a MIDI input for that. ({see example}[http://github.com/arirusso/diamond/blob/master/examples/midi_input.rb]).  However, for the sake of simplicity here is a three note chord created with Ruby code
...
Use Arpeggiator#add and Arpeggiator#remove to adjust what notes the arpeggiator sees
...
The video above shows Diamond being live coded in this way  

Also, {Here is a blog post}[] that walks through that process
