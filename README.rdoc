...
An arpeggiator is a type of music sequencer that derives melodies from inputted notes and/or chords.
...
The Arpeggiator has a number of options that you can control. {Click here}[options] for a more in-depth explanation of those options.
...
And of course, the Arpeggiator needs input notes
...
If you are using this example in irb, you can start the arpeggiator in a background thread.  That will allow you to change its parameters on the fly or sync it to another arpeggiator. 
   
  arp.start(:background => true)
  
Read more about live coding with diamond {here}[blogpost].
  
To start the arpeggiator in the foreground simply call
