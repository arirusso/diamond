= diamond

MIDI arpeggiator in Ruby

An arpeggiator is a type of music sequencer that derives melodies from inputted notes and/or chords.

== Installation

  gem install diamond
  
== Gem dependencies

* {midi-message}[http://github.com/arirusso/midi-message]
* {topaz}[http://github.com/arirusso/topaz]
* {unimidi}[http://github.com/arirusso/unimidi]

these will install automatically with the gem

== Usage

  require "diamond"

the arpeggiator has a number of options that you can control. click here for a more in-depth explanation of those options.
 
  opts = { 
    :gate => 90, 
    :range => 4, 
    :interval => 7,
    :midi => UniMIDI::Output.first.open,
    :rate => 8
  }

  arp = Diamond::Arpeggiator.new(175, opts)

  chord = [
    MIDIMessage::NoteOn["C4"].new(0, 100),
    MIDIMessage::NoteOn["E4"].new(0, 100),
    MIDIMessage::NoteOn["G4"].new(0, 100)
  ]

You can use Arpeggiator#add and Arpeggiator#remove to control the input notes and chords.

  arp.add(chord)
  
If you are using this example in irb, you can start the arpeggiator in a background thread.  That will allow you to change its parameters on the fly or sync it to another arpeggiator. 
   
  arp.start(:background => true)
  
Read more about live coding with diamond {here}[blog post].
  
You can also start the arpeggiator in the foreground simply by calling

  arp.start  
    
== Documentation

* {examples}[http://github.com/arirusso/diamond/tree/master/examples]
* {rdoc}[http://rdoc.info/gems/diamond] 

== Author

* {Ari Russo}[http://github.com/arirusso] <ari.russo at gmail.com>

== License

Apache 2.0, See the file LICENSE

Copyright (c) 2011 Ari Russo
