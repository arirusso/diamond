...
Of course, an Arpeggiator needs notes to work with. As you might expect, it is easy to use a MIDI input for that. ({see example}[http://github.com/arirusso/diamond/blob/master/examples/midi_input.rb]). However, again for the sake of simplicity here is a chord in Ruby
...
The screencast video above shows Diamond being live coded in this way. ({Read how this is done}[blog link])

==== Links:

* {Example of syncing Diamond to external MIDI clock}[http://github.com/arirusso/diamond/blob/master/examples/midi_clock.rb]
* {Example of feeding notes to Diamond using a MIDI controller or other input}[http://github.com/arirusso/diamond/blob/master/examples/midi_input.rb]
* {Example of feeding notes to Diamond using MIDI message objects}[http://github.com/arirusso/diamond/blob/master/examples/midi_message.rb]
* {Live coding Diamond and chaining multiple arpeggiators}[blog link] 

== Other Documentation

* {all examples}[http://github.com/arirusso/diamond/tree/master/examples]
