...
If you'd like to sync to MIDI clock instead of internally generating tempo, pass in a unimidi input instead of a numeric tempo ({see example}[http://github.com/arirusso/diamond/blob/master/examples/midi_sync.rb]).  MIDI Clock will always be sent from Diamond to any outputs that are passed in
...
{Here is a blog post}[] that walks through the process
